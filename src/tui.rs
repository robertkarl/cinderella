                ...
                Event::Key(key) => {
                    match handle_key(key, &mut state) {
                        KeyAction::Send => {
                            ...
                            if msg == "/help" {
                                state.entries.push(ChatEntry::Info(HELP_TEXT.to_string()));
                                continue;
                            }

                            state.entries.push(ChatEntry::UserMessage(msg.clone()));
            ...
            match event {
                ...
                AgentEvent::ToolStart { name, args_display } => {
                    ...
                    if !state.streaming_text.is_empty() {
                        state
                            .entries
                            .push(ChatEntry::AssistantText(state.streaming_text.clone()));
                        state.streaming_text.clear();
                    }
                ...
                AgentEvent::Warning(msg) => {
                    state.entries.push(ChatEntry::Warning(msg));
                }
                ...
                AgentEvent::ServerRestarted { attempt } => {
                    state.entries.push(ChatEntry::Warning(format!(
...
fn render(frame: &mut Frame, state: &TuiState) {
    ...
    // Title bar
    let title = format!(" cinderella v0.1.0 \u{00b7} {} ", "project");
    ...
    if !state.is_processing {
        let cursor_x = chunks[2].x + state.input_cursor as u16;
